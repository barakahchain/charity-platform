...
export default function CharityProjectCreation() {
  ...
  const FORM_STORAGE_KEY = "charity_project_draft";
  ...
  const sendWithRetries = async (
    ...
    while (attempt <= maxRetries) {
      try {
        if (attempt > 0) {
          toast.info(`Retrying transaction… (${attempt}/${maxRetries})`);
        }
        ...
        // ---------------------------
        // ❌ User rejected the request → do NOT retry
        // ---------------------------
        const userRejected =
          ...
          err?.cause?.code === 4001 || // EIP-1193 standard code
          err?.code === 4001;

        if (userRejected) {
          console.warn("User rejected transaction. No retries.");
        ...
        if (attempt > maxRetries) {
          console.error("Final transaction failure:", err);
          throw err;
        }
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setZakatMode(data.zakatMode || false);
        setAsnafTag(data.asnafTag || "");
        setContractTemplate(data.contractTemplate || "wakalah");
        setGoalAmount(data.goalAmount || "");
        setDeadline(data.deadline || "");
        setDeadlineEnabled(data.deadlineEnabled || false);
        setMilestones(
          data.milestones || [
            { id: "1", description: "", amount: "", beneficiaryAddress: "" },
          ]
  ...
  useEffect(() => {
    if (!loadedDraft) return; // ⛔ Prevent overwriting on initial load
      ...
      if (
        ...
        try {
          ...
          if (receipt.status === "success") {
            ...
            // Extract project address from transaction receipt
            const projectContractAddress = getProjectCreatedAddress(receipt);

            // Save project to database
            if (metadataCid) {
              await saveProjectToDB(
                projectContractAddress || "unknown",
                metadataCid
              );
            }
  ...
  const testWithSimpleParameters = async () => {
    try {
      ...
      if (!isCorrectNetwork) {
        throw new Error(
          `Please switch to Polygon Amoy. Current network: ${chain?.name}`
        );
      }
      ...
      if (error?.cause?.data?.args?.[0]) {
        ...
        toast.error(
          "Contract execution failed. This usually means:\n• Contract validation failed\n• Insufficient permissions\n• Invalid parameters"
        );
  ...
  // In your CharityProjectCreation component
  const saveProjectToDB = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn("⚠️ Could not save project to database:", result.error);

        // Handle specific error cases
        if (response.status === 400) {
          if (result.error === "Deadline required") {
            toast.error(
              "Deadline is required when deadline enforcement is enabled."
            );
          } else if (result.error === "Invalid deadline") {
            toast.error("Deadline must be in the future.");
          } else {
          ...
          toast.error(
            "Wallet not found. Please ensure your wallet is properly connected and registered."
          );
        } else if (response.status === 403) {
          toast.error(
            "Insufficient permissions. Only charity accounts can create projects."
          );
        } else if (response.status === 409) {
          toast.error("Project already exists with this contract address.");
        } else {
          toast.warning(
            "Project created on blockchain but failed to save to database"
          );
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (debugMode) {
        ...
        const debugHash = await sendWithRetries(() =>
          testWithSimpleParameters()
        );
        ...
        const formHash = await sendWithRetries(() =>
          writeContractAsync({
            address: FACTORY_CONTRACT_ADDRESS as `0x${string}`,
            abi: ProjectFactoryABI,
            functionName: "createProject",
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Button
                  ...
                  disabled={
                    isSubmitting || isPending || isConfirming || isConfirmed
                  }
                >
                ...
                <Button
                  ...
                  disabled={isSubmitting || isConfirming}
                >
                  {isConfirmed ? "New Project" : "Reset Form"}
                </Button>
              </div>
            </div>
          </CardContent>
